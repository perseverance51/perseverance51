...
void loop() {
  ...
  if (millis() - timeSinceLastWUpdate > (1000L * UPDATE_Weather_SECS)) { //天气更新频率
    ...
    unsigned int sece =  timeClient.getSeconds();
...
void updateDatas(OLEDDisplay *display) {  //首次天气、预报天气，农历的整体更新，重启时才调用
  ...
  HeFengClient.doUpdateFore(foreWeather, HEFENG_KEY, HEFENG_LOCATION);
  delay(1500);
  drawProgress(display, 0, "wellcome！...");
  HeFengClient.NongLingDate(&NongLi, DateAddress);
  delay(200);
...
void setReadyForUpdate() {  //为3天更新做好准备
  Serial.println("Setting ready 3Day Update to true");
  readyForUpdate = true;
}
void setReadyNLiUpdate() {  //为农历更新做好准备
  Serial.println("Setting readyNLiUpdate to true");
  readyNLiUpdate = true;
}

void drawDateTime2(OLEDDisplay * display, OLEDDisplayUiState * state, int16_t x, int16_t y) { //显示时间
  display->clear();
  now = time(nullptr);
  struct tm* timeInfo;
  timeInfo = localtime(&now);
