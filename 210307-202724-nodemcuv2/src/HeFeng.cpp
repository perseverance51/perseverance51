  ...
  { // HTTPS
    ...
    if (httpCode > 0)
    {
      ...
      if (((httpCode == HTTP_CODE_OK) || (httpCode == HTTP_CODE_MOVED_PERMANENTLY)))
      {
        ...
        Serial.println("Nong Li"); //打印json数据
  ...
  { // HTTPS
    ...
    if (httpCode > 0)
    {
      if ((httpCode == HTTP_CODE_OK) || (httpCode == HTTP_CODE_MOVED_PERMANENTLY))
      {
        ...
        for (int i = 0; i < 10; i++)
        {
          Readate += char(EEPROM.read(101 + i));
          delay(15);
        }
        Serial.println("eeprm Address:" + Readate);
        String Today = doc["sysTime2"]; //  "2021-03-03 01:28:24"
        String Todaycut = Today.substring(0, 10);
        char buf[11] = {0};
        Todaycut.toCharArray(buf, 11);
        Serial.println(buf);                     //年月日
        Serial.println(Today.substring(11, 19)); //时分秒
        if (!(Readate.equals(Todaycut)))
        {
          for (int i = 0; i < 11; i++)
          {
            EEPROM.write(101 + i, Todaycut[i]);
            delay(15);
          }
          EEPROM.commit();
          delay(30);
          Serial.println(F("eeprom中的数据与json数据不相等"));
        }
    ...
    {
      Serial.println(F("GET失败！以下为EEPROM读取的信息："));
      EEPROM.begin(156);
      String ReadToday = "";
      for (int i = 0; i < 12; i++)
      {
        ReadToday += EEPROM.read(101 + i);
        delay(15);
      }
      EEPROM.commit();
      Serial.print(ReadToday);
      data->workdate = ReadToday;
    }
    http.end();
  }
}
