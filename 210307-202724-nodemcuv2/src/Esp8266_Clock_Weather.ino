...
const unsigned int UPDATE_Today_SECS = 60 * 60;      //日期更新频率
...
void loop()
{
  ...
  { //温度更新频率
    ...
    {
      currTemp = String(ds.getTempC() - 6.13, 2); //浮点数转字符串，2表示小数点精度？
  ...
  { //NTP更新频率
    ...
    Serial.print("距第二天剩余秒数:" + String("") + T2);
    Serial.print(String("") + "星期:" + TodayDay);
...
{                                     //显示当天实时气象天气
  ...
  display->setTextAlignment(TEXT_ALIGN_CENTER);
  display->drawString(32 + x, y - 2, currentWeather.iconMeteoCon);
...
{ //显示中文星期和时间
  ...
  case 0:
    (*display).drawXbm(76, 5, 16, 16, ri2);
    break;
  case 1:
